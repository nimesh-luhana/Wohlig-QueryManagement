...
<div class="form-row control-group row-fluid">
<label class="control-label span1" for="description-field">Customer</label>
  <div class="controls span3"> <?php echo form_dropdown('customer',$customer,set_value('customer'),'class="chzn-select customer"'); ?> </div> 
</div>
...
var form_data = {
				...
				console.log(form_data)
				$.ajax({
				url: "<?php echo site_url('site/salesreportdata'); ?>",
				type: 'POST',
				data: form_data,
				success: function(msg) {
					console.log(msg);
					...
					$('.tablebody').append('<tr><td>'+message[i].date+'</td><td>'+message[i].shopname+'</td><td>'+message[i].customername+'</td><td>'+message[i].categoryname+'</td><td>'+message[i].productname+'</td><td>'+message[i].totalcost+'</td><td>'+message[i].quantity+'</td><td>'+message[i].total+'</td><td style="display:none;">'+message[i].taxamount+'</td><td style="display:none;">'+message[i].finaltotal+'</td></tr>');
					}
